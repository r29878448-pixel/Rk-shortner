  ...
  return (
    <div className="bg-white min-h-[75vh] flex flex-col justify-center py-12 px-6 font-sans">
      <div className="max-w-4xl mx-auto text-center">
        ...
        <form onSubmit={handleShorten} className="bg-white p-2 md:p-3 rounded-2xl border-2 border-slate-100 flex flex-col md:flex-row gap-2 md:gap-3 max-w-2xl mx-auto mb-12 md:mb-20 shadow-xl shadow-slate-100/50">
          <div className="relative flex-grow">
            <LinkIcon className="absolute left-4 md:left-5 top-1/2 -translate-y-1/2 text-slate-400 w-5 h-5" />
            <input 
              type="url" 
              placeholder="Paste long URL here..." 
              className="w-full pl-12 md:pl-14 pr-6 py-4 md:py-6 rounded-xl bg-slate-50 border-none focus:ring-4 focus:ring-indigo-100 focus:outline-none font-bold text-slate-700 transition"
              value={url}
              onChange={(e) => setUrl(e.target.value)}
              required
            />
          </div>
          <button className="bg-indigo-600 text-white px-8 md:px-12 py-4 md:py-6 rounded-xl font-black uppercase text-sm tracking-widest shadow-xl shadow-indigo-200 hover:bg-indigo-700 transition-all flex items-center justify-center min-w-[200px] active:scale-95">
            {isLoading ? <Loader2 className="animate-spin w-6 h-6" /> : 'Shorten URL'}
          </button>
        </form>

        {shortenedLink && (
          <div className="bg-slate-900 p-6 md:p-10 rounded-2xl text-white flex flex-col md:flex-row items-center justify-between gap-6 max-w-2xl mx-auto animate-in border-4 border-slate-800 shadow-2xl">
            ...
            <button onClick={copyToClipboard} className="w-full md:w-auto bg-white text-slate-900 px-8 py-5 rounded-xl font-black text-xs uppercase tracking-widest flex items-center justify-center space-x-3 shrink-0 hover:bg-indigo-50 transition active:scale-95 shadow-lg">
